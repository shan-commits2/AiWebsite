...
export function AnalyticsDashboard() {
  ...
  const recentStats = usageStats.slice(-7); // Last 7 days
      ...
      <DialogContent className="sm:max-w-4xl bg-gray-800 border-gray-700 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">
            Track your AI chat usage and performance metrics
          </DialogDescription>
        </DialogHeader>
                ...
                {Object.entries(
                  ...
                  <div key={model} className="flex items-center justify-between">
                    <span className="text-gray-300 capitalize">
                      {model.replace('-', ' ').replace('gemini', 'Gemini')}
                    </span>
                    <div className="flex items-center space-x-3">
                      <div className="w-24 bg-gray-600 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${Math.min(100, (count / Math.max(...Object.values(usageStats.reduce((acc, stat) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <ul className="space-y-1 text-sm text-gray-400">
                  <li>• Average conversation length: {totalConversations > 0 ? Math.round(totalMessages / totalConversations) : 0} messages</li>
                  <li>• Peak usage: {recentStats.length > 0 ? Math.max(...recentStats.map(s => s.messagesExchanged)) : 0} messages/day</li>
                  <li>• Most active model: {usageStats.length > 0 ? Object.keys(usageStats[0]?.modelsUsed || {})[0] || 'N/A' : 'N/A'}</li>
                </ul>
              </div>
